    ...
    .then((data) => {
      ...
      for (let i = 110; i < 150; i++) {
        ...
        <img src="${makeup.image_link}" class="click image" alt="image"  id="${makeup.id}">
             ...
             <h4 class="Makeup_Price" id="price">$${makeup.price} <span>(20 % off)</span></h4>
...
container.addEventListener("click", function productShow(e){
  ...
  if (e.target.classList[0] == "click" ||e.target.classList[0] == "Makeup_Name" || e.target.classList[0] == "box" || e.target.classList[0] == "Makeup_Price") {
      // console.log(e.target.id)
      alert(e.target.textContent)
      localStorage.setItem("localimage",e.target.src)
      localStorage.setItem("local_name",e.target.textContent)
      // localStorage.setItem("local_price",e.target.textContent)
